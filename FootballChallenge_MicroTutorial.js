/**************************************** 
 * Footballchallenge_Microtutorial *
 ****************************************/
...
let expName = 'FootballChallenge_MicroTutorial';  // from the Builder filename that created this script
let expInfo = {
    'nombor maktab': '',
};
...
function SS_instruction_loopLoopBegin(SS_instruction_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    for (const thisSS_instruction_loop of SS_instruction_loop) {
...
function SS_main_loopLoopBegin(SS_main_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisSS_main_loop of SS_main_loop) {
...
function SS_trials_loopLoopBegin(SS_trials_loopLoopScheduler, snapshot) {
  return async function() {
    ...
    // Schedule all the trials in the trialList:
    for (const thisSS_trials_loop of SS_trials_loop) {
...
function SS_instructionRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of SS_instructionComponents)
...
function SS_ISIRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of SS_ISIComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function SS_trialsRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of SS_trialsComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function SS_break_or_endRoutineBegin(snapshot) {
  return async function () {
    ...
    for (const thisComponent of SS_break_or_endComponents)
      if ('status' in thisComponent)
        thisComponent.status = PsychoJS.Status.NOT_STARTED;
    return Scheduler.Event.NEXT;
  }
...
function datapipeRoutineBegin(snapshot) {
  return async function () {
    ...
        },
        body: JSON.stringify({
          experimentID: 'sQi4RwkJCbcw', // <-- UPDATE WITH YOUR DATAPIPE EXPERIMENT ID
          filename: filename,
          data: data
        })
      })
